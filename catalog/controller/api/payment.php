...
class ControllerApiPayment extends Controller {
	public function address() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ((utf8_strlen(trim($this->request->post['address_1'])) < 2) || (utf8_strlen(trim($this->request->post['address_1'])) > 128)) {
			...
			if (!$json) {
				...
				foreach ($results as $result) {
					if ($this->config->get($result['code'] . '_status')) {
						...
						$method = $this->{'model_payment_' . $result['code']}->getMethod(null, $total);
