...
class ModelMarketingAffiliate extends Model {
	...
	public function getAffiliates($data = array()) {
		$sql = "SELECT *, a.firstname AS name, (SELECT SUM(at.amount) FROM " . DB_PREFIX . "affiliate_transaction at WHERE at.affiliate_id = a.affiliate_id GROUP BY at.affiliate_id) AS balance FROM " . DB_PREFIX . "affiliate a";

		$implode = array();

		if (!empty($data['filter_name'])) {
			$implode[] = "a.firstname LIKE '" . $this->db->escape($data['filter_name']) . "%'";
		}
	...
	public function getAffiliatesByNewsletter() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "affiliate WHERE newsletter = '1' ORDER BY firstname, email");
