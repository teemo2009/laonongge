...
class ModelSaleRecurring extends Model {
	public function getTotalRecurrings($data) {
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " o.firstname LIKE '" . $this->db->escape($data['filter_customer']) . "%'";
		}
	...
	public function getRecurrings($data) {
		$sql = "SELECT `or`.order_recurring_id, `or`.order_id, `or`.reference, `or`.`status`, `or`.`date_added`, `o`.`firstname` AS `customer` FROM `" . DB_PREFIX . "order_recurring` `or` JOIN `" . DB_PREFIX . "order` `o` USING(`order_id`) WHERE 1 = 1 ";
