...
class ModelReportCustomer extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT c.customer_id, c.firstname AS customer, c.email, cgd.name AS customer_group, c.status, o.order_id, SUM(op.quantity) as products, SUM(DISTINCT o.total) AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id)LEFT JOIN `" . DB_PREFIX . "customer` c ON (o.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE o.customer_id > 0 AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getCredit($data = array()) {
		$sql = "SELECT ct.customer_id, c.firstname AS customer, c.email, cgd.name AS customer_group, c.status, SUM(ct.amount) AS total FROM `" . DB_PREFIX . "customer_transaction` ct LEFT JOIN `" . DB_PREFIX . "customer` c ON (ct.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getCustomersOnline($data = array()) {
		...
		if (!empty($data['filter_customer'])) {
			$implode[] = "co.customer_id > 0 AND c.firstname LIKE '" . $this->db->escape($data['filter_customer']) . "'";
		}
	...
	public function getCustomerActivities($data = array()) {
		...
		if (!empty($data['filter_customer'])) {
			$implode[] = "c.firstname LIKE '" . $this->db->escape($data['filter_customer']) . "'";
		}
