...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			if ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) {
				$unlock = $this->url->link('customer/customer/unlock', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL');
			} else {
				$unlock = '';
			}
			...
			$data['customers'][] = array(
				...
				'name'           => trim($result['name']) ? $result['name'] : (trim($result['email']) ? $result['email'] : $result['telephone']),
				'email'          => $result['email'],
	...
	protected function validateForm() {
		...
		if (!(isset($this->request->post['email']) && $this->request->post['email']) &&  !(isset($this->request->post['telephone']) && $this->request->post['telephone'])) {
			$this->error['email'] = $this->language->get('error_email_and_telephone_cannot_all_null');
			$this->error['telephone'] = $this->language->get('error_email_and_telephone_cannot_all_null');
		}
		...
		if (isset($this->request->post['address'])) {
			foreach ($this->request->post['address'] as $key => $value) {
				...
				if ((utf8_strlen($value['address_1']) < 2) || (utf8_strlen($value['address_1']) > 128)) {
